document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadArticleContent() {
        try {
            ...
            const article = articleData['b1'].articles.find(a => a.link === 'article1.html');
            if (article) {
                ...
                document.getElementById('article-author').textContent = `${article.author || 'Unknown'}`;
                document.getElementById('article-content').innerHTML = article.content ? marked.parse(article.content) : 'Content is currently unavailable.';
